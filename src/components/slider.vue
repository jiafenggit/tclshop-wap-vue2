...
<template>
  <div class='slider-container' :class='basicdata.containerClass'>
    <div class="slider-wrapper" :style="styleobj" :class="basicdata.animation" @touchmove="swipeMove" @touchstart="swipeStart"
      @touchend="swipeEnd" @mousedown="swipeStart" @mouseup="swipeEnd" @mousemove="swipeMove">
      <!-- 正常滚动 -->
      <template v-if="!sliderinit.loop">
        <template v-for="item in pages">
          <div class="slider-item" :style="item.style"></div>
        </template>
      </template>
      <!-- 无缝滚动 -->
      <!--<template v-if="sliderinit.loop">
        ...
        get: function () {
          ...
          for (let item in $sliderChildren) {
            if (item <= lastPage) {
              ...
              poswidth += parseInt($sliderChildren[item].style.marginRight.length ? $sliderChildren[item].style.marginRight :
                0);
              poswidth += parseInt($sliderChildren[item].style.marginLeft.length ? $sliderChildren[item].style.marginLeft :
                0);
            }
          }
          return poswidth
      ...
      currentHeight: function () {
        ...
        for (let item in $sliderChildren) {
          if (item <= lastPage) {
            ...
            posheight += parseInt($sliderChildren[item].style.marginTop.length ? $sliderChildren[item].style.marginTop :
              0);
            posheight += parseInt($sliderChildren[item].style.marginBottom.length ? $sliderChildren[item].style.marginBottom :
              0);
          }
        }
        return posheight
